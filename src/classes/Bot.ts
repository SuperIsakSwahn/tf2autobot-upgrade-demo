...
export default class Bot {
    ...
    messageAdmin(...args: [string, string[] | SteamID[]] | [string, string, string[] | SteamID[]]): void {
        ...
        // pick the first admin that is not excluded
        const targetAdmin = this.admins.find(steamID => !exclude.includes(steamID.toString()));
    ...
    async checkUpgradeUpdates(anyoneAsked=false): Promise<void> {
        try {
            ...
            const currentVersion = '1.13'; // your local version number
            ...
            if (content.version !== currentVersion) {
                ...
                const discordId = this.getFirstAdminDiscordId();
                if (discordId && this.discordBot) {
                    await this.discordBot.sendDMToUser(discordId, dmText).catch(err => log.warn('Failed to DM admin', err));
                }
                this.messageAdmin(
                    'version',
                    `⚠️ tf2autobot-upgrade-demo update available! Current: v${currentVersion}, Latest: v${content.version}\n\n💬 Download from here: https://github.com/SuperIsakSwahn/tf2autobot-upgrade-demo.`,
                    []
                );
            } else if (anyoneAsked) {
                this.messageAdmin(
                    'version',
                    `✅ You are running the latest version of the upgrade (${currentVersion}).`,
                    []
                );
            }
        } catch (err) {
            log.error('Failed to check upgrade updates:', err);
        }
    }
