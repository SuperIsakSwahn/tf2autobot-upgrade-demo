...
export default class TF2GC {
    ...
    handleSortJob(job: Job, steamID?: string): void {
        log.debug('TF2GC line: Sending sort request');
        ...
        const cancel = this.listenForEvent(
            ...
            () => {
                ...
                timeout = setTimeout(() => {
                    ...
                    // Only send message if steamID was provided
                    if (steamID) {
                        this.bot.sendMessage(steamID, 'Successfully sorted!');
                    }
                }, 8000);
                return { success: false, clearTimeout: true };
            },
            () => {
                log.debug('TF2GC line: Successfully sorted')
                this.finishedProcessingJob();
                if (steamID) {
                    this.bot.sendMessage(steamID, 'Successfully sorted!');
                }
            ...
            err => {
                if (err.message === 'Canceled') {
                    log.debug('TF2GC line: Was canceled because of timeout')
                    this.finishedProcessingJob();
                    if (steamID) {
                        this.bot.sendMessage(steamID, 'Sorting timed out!');
                    }
                } else {
                    log.debug('TF2GC line: Job failed')
                    this.finishedProcessingJob(err);
                    if (steamID) {
                        this.bot.sendMessage(steamID, 'Sorting failed: ' + err.message);
                    }
                }
            }
        );
