...
export default class MyHandler extends Handler {
    ...
    // These are the fallback prices in scraps. 9 = 1 ref.

    valuableWeapons = { // This would be slower, but you can use the await pricelist lookup function for each of these items
        '587;': 23,     // The Apoco-Fists
        '1013;': 1,    // The Ham Shank
        '474;': 23,     // The Conscientious Objector
        '939;': 1,     // The Bat Outta Hell
        '880;': 1/2,     // The Freedom Staff
        '638;': 23,     // The Sharp Dresser
        '574;': 1,     // Wanga Prick
        '30474;': 23,   // The Nostromo Napalmer
        '466;': 23,     // The Maul
        '452;': 23,     // Three-Rune Blade
        '572;': 1,     // Unarmed Combat
        '851;': 27,     // The AWPer Hand
        '947;': 12,     // Quäckenbirdt
        '411;': 2,     // Quick-Fix
        '998;': 9      // Vaccinator
    };
    ...
    getQualityFromSKU(sku: string): number {
    ...
    async onNewTradeOffer(offer: TradeOffer): Promise<null | OnNewTradeOffer> {
        ...
        function incrementFilenameCounter(dir: string, amount: number): void {
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true });
            }

            const files = fs.readdirSync(dir);
            const existingFile = files.find(name => /^\d+$/.test(name));

            let currentCount = 0;

            if (existingFile) {
                currentCount = parseInt(existingFile, 10);
                fs.unlinkSync(path.join(dir, existingFile)); // Remove old file
            }

            const newCount = currentCount + amount;
            fs.writeFileSync(path.join(dir, String(newCount)), ''); // Write empty file with new number as name
        }
        ...
        for (let i = 0; i < states.length; i++) {
                ...
                for (const id of exchange[which].pricedAssetIds) {
                    ...
                    if (which === 'their') {
                        ...
                        if (itemPrice == 0) {
                            ...
                            // ✅ Will be true if id starts with any of those (ex: "998;6", "998;6;kt-1")
                            ...
                            const isFestivized = id.includes('festive');
                            ...
                            if (isCosmetic) value += 9;
                            if (isCraftOnlyWeapon) value += this.valuableWeapons[weaponPrefix];

                            hasNoPrice = true;
                            exchange['their'].scrap += value;
                            exchange['their'].value += value;
                        } else {
            ...
            for (const sku in itemsDict[which]) {
                ...
                if (sku === '5000;6') {
                    ...
                    if (match !== null) {
                        const itemValue = match[intentString];
                        ...
                        if (which === 'our') {
                            ...
                            if (itemPrice == 0) {
                                ...
                                if (isCosmetic) value += 9;
                                if (isCraftOnlyWeapon) value += this.valuableWeapons[weaponPrefix];
                                exchange[which].value += value * amount;

                            } else {
                                exchange[which].value += itemPrice * amount;
                            }
                        }
                        ...
                        if (!isNonTF2Items) {
                            ...
                            if (/* price === null*/ 1 === 1) {
                                hasNoPrice = true;
                                // const isCosmetic = item && (item.item_slot === "head" || item.item_slot === "misc" || item.item_slot === "miscellaneous");
                                if (which === 'their') {

                                    // ✅ Will be true if sku starts with any of those (ex: "998;6", "998;6;kt-1")
                                    const weaponPrefix = Object.keys(this.valuableWeapons).find(prefix => sku.startsWith(prefix));
                                    const isCraftOnlyWeapon = weaponPrefix !== undefined;
                                    const isFestivized = sku.includes('festive');
                                    const isAustralium = sku.includes('australium');
                                    ...
                                    exchange['their'].scrap += value * amount;
                                    exchange['their'].value += value * amount;
                                    if (value !== 0) {
                                        itemSuggestedValue = `Not priced - Using ${(value / 9).toFixed(2).replace('.00', '')} ref fallback`;
                                    }
                                }
                            } else {
