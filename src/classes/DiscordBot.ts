...
// Bot.ts (inside class)
    private getFirstAdminDiscordId(): string | null {
        try {
            for (const admin of this.admins) {
                // admin.discordID comes from earlier: admin.discordID = adminData.discord;
                if (admin && admin.discordID) {
                    return String(admin.discordID);
                }
            }
        } catch (err) {
            log.warn('getFirstAdminDiscordId failed', err);
        }
        return null;
    }

    // New: send DM to a user by Discord snowflake id
    public async sendDMToUser(discordId: string, message: string): Promise<boolean> {
        if (!this.client) {
            log.warn('DiscordBot: client not initialized, cannot send DM.');
            return false;
        }
        try {
            const user = await this.client.users.fetch(discordId);
            if (!user) {
                log.warn(`DiscordBot: user ${discordId} not found.`);
                return false;
            }
            await user.send(message);
            log.info(`Discord DM sent to ${discordId}: ${message}`);
            return true;
        } catch (err) {
            log.error(`DiscordBot: Failed to DM ${discordId}:`, err);
            return false;
        }
    }
    ...
    protected sendMessage(origMessage: Message, message: string): void {
        ...
        (origMessage.channel as TextChannel)
            .send(message)
            .then(() => log.info(`Discordmessage sent to ${origMessage.author.tag} (${origMessage.author.id}): ${message}`))
