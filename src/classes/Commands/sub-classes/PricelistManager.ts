...
export default class PricelistManagerCommands {
    ...
    async addCommand(steamID: SteamID, message: string): Promise<void> {
        ...
        if (params.sku === undefined) {
            if (params.item !== undefined) {
                params.sku = this.bot.schema.getSkuFromName(params.item as string);
                params.sku1 = this.bot.schema.getSkuFromName(params.item.slice(0,1) as string);
                params.sku2 = this.bot.schema.getSkuFromName(params.item.slice(0,2) as string);

                if ((params.sku as string).includes('null') || (params.sku as string).includes('undefined')) {
                    params.sku = params.sku1;
                    if ((params.sku as string).includes('null') || (params.sku as string).includes('undefined')) {
                        params.sku = params.sku2;
                        if ((params.sku as string).includes('null') || (params.sku as string).includes('undefined')) {
                            return this.bot.sendMessage(
                                steamID,
                                `❌ The sku for "${params.item as string}" returned "${params.sku as string}".` +
    ...
    async addbulkCommand(steamID: SteamID, message: string): Promise<void> {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!(itemToAdd.includes('item=') || itemToAdd.includes('sku=') || itemToAdd.includes('name=') ||
                itemToAdd.includes('id=') || itemToAdd.includes('defindex='))) itemToAdd = `item=${itemToAdd}`;
    ...
    async updateCommand(steamID: SteamID, message: string, prefix: string): Promise<void> {
        if (!(message.includes('item=') || message.includes('sku=') || message.includes('name=') ||
            message.includes('id=') || message.includes('defindex='))) {

            // Split the message into first word (the command) and the rest
    ...
    async updatebulkCommand(steamID: SteamID, message: string): Promise<void> {
        ...
        for (let i = 0; i < count; i++) {
            let itemToUpdate = itemsToUpdate[i];
            if (!(itemToUpdate.includes('item=') || itemToUpdate.includes('sku=') || itemToUpdate.includes('name=') ||
                itemToUpdate.includes('id=') || itemToUpdate.includes('defindex='))) itemToUpdate = `item=${itemToUpdate}`;
    ...
    async removeCommand(steamID: SteamID, message: string): Promise<void> {
        ...
        if (params.all === true) {
            ...
            if (params.withgroup && params.withoutgroup) {
                return this.bot.sendMessage(
                    steamID,
                    `❌ Don't be dumb. Please choose only "withgroup" OR "withoutgroup", not both. Thanks.` // What rude cunt wrote that?
                );
            }
    ...
    removebulkCommand(steamID: SteamID, message: string): void {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!(itemToRemove.includes('item=') || itemToRemove.includes('sku=') || itemToRemove.includes('name=') ||
                itemToRemove.includes('id=') || itemToRemove.includes('defindex='))) itemToRemove = `item=${itemToRemove}`;
